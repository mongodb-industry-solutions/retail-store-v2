import { COLLECTIONS, shippingMethods } from "./constants";
...
export async function fetchInvoice(invoiceId) {
  const response = await fetch("/api/findDocuments", {
    ...
    body: JSON.stringify({filter: {_id: invoiceId}, collectionName: 'invoices'}),
  });
  if (!response.ok) {
    throw new Error(`Error fetching cart: ${response.status}`);
  }
...
async function fetchproduct(productId) {
  ...
  const response = await fetch("/api/findDocuments", {
    ...
    body: JSON.stringify({filter: {_id: productId}, collectionName: 'products'}),
  });
  if (!response.ok) {
    throw new Error(`Error fetching cart: ${response.status}`);
  }
...
export async function fetchCustomerBehaviours() {
  // Get sessionId and userId from Redux store
  const userId = store.getState().User.selectedUser?._id;
  const sessionId = sessionStorage.getItem('sessionId');
  
  if (!userId || !sessionId) {
    console.warn('Missing userId or sessionId for fetching customer behaviours');
    return [];
  }
