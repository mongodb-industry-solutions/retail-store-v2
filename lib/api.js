...
export async function getProductsWithSearch(query = "", filters = {}) {
  ...
  const searchType = store.getState("Products").Products.searchType || "vector";
...
export async function createNewOrder(
  ...
  if (!response.ok) {
    if (response.status === 409) {
      const msg =
        data.code === "NO_INVENTORY"
          ? "A product in your cart has no inventory record. The store owner can seed inventory from the Inventory page."
          : data.code === "INSUFFICIENT_STOCK"
            ? `Not enough stock for an item in your cart (requested ${data.requested}, available ${data.available}).`
            : data.error || "Could not complete order.";
      throw new Error(msg);
    }
    throw new Error(data.error || `Error creating order: ${response.status}`);
  }
  return data.order;
}
...
export async function fetchproduct(productId) {
  const idStr =
    productId != null && typeof productId === "object" && productId.toString
      ? String(productId)
      : String(productId);

  const products = store.getState().Products.products;
  const cached = Array.isArray(products)
    ? products.find((p) => String(p._id) === idStr)
    : null;

  if (cached && Object.prototype.hasOwnProperty.call(cached, "stockQuantity")) {
    return cached;
  }
  ...
  if (!doc) {
    ...
    const data = await response.json();
    console.log(data);
    doc = data.result.length > 0 ? data.result[0] : null;
  }
  if (!doc) return null;

  const invRes = await fetch(`/api/inventory/${String(doc._id)}`);
  if (invRes.ok) {
    const inv = await invRes.json();
    return { ...doc, stockQuantity: inv.quantity };
  }
  return { ...doc, stockQuantity: null };
}
...
export async function fetchCatalogProducts(actingUserId, productId) {
  const response = await fetch("/api/catalog/products", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ actingUserId, productId }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.error || `Catalog products: ${response.status}`);
  }
  return data;
}

export async function createCatalogProduct(actingUserId, product) {
  const response = await fetch("/api/catalog/product", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ actingUserId, product }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.error || `Create product: ${response.status}`);
  }
  return data.product;
}

export async function updateCatalogProduct(actingUserId, productId, fields, unsetKeys) {
  const response = await fetch("/api/catalog/product", {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ actingUserId, productId, fields, unsetKeys }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.error || `Update product: ${response.status}`);
  }
  return data;
}

export async function deleteCatalogProduct(actingUserId, productId) {
  const response = await fetch("/api/catalog/product", {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ actingUserId, productId }),
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.error || `Delete product: ${response.status}`);
  }
  return data;
}

export async function fetchInventoryProducts(actingUserId) {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.error || `Inventory list: ${response.status}`);
  }
  return data;
}

export async function patchInventoryProduct(actingUserId, productId, quantity) {
  const response = await fetch("/api/inventory", {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ actingUserId, productId, quantity }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.error || `Inventory update: ${response.status}`);
  }
  return data;
}

export async function seedInventory(actingUserId) {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.error || `Inventory seed: ${response.status}`);
  }
  return data;
}
...
export async function fetchInventoryMovements(
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.error || `Inventory movements: ${response.status}`);
  }
  return data;
}
