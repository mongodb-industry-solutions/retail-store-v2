...
const DigitalReceiptComp = () => {
    ...
    // ✅ Portal container for LeafyGreen GuideCue  
    const [portalContainer, setPortalContainer] = useState(null);

    // ✅ Build config using constants + refs  
    const currentConfig = useMemo(() => {
        if (feature === FEATURES.RECEIPTS) {
            const messages = GUIDE_CUE_MESSAGES_2.digitalReceipt.receipts.messages;
            const triggers = [triggerRef1, triggerRef2, triggerRef3, triggerRef4];
            return {
                ...GUIDE_CUE_MESSAGES_2.digitalReceipt.receipts,
                triggers,
    ...
    // ✅ Set portal container only on client side  
    useEffect(() => {
    ...
    // ✅ Start walkthrough when feature is active and update portal container  
    useEffect(() => {
        if (openedInvoice !== null && feature === FEATURES.RECEIPTS) {
            setTimeout(() => {
                ...
                handleReset();
                console.log('🚀 Starting digital receipt walkthrough for feature:', feature);
            }, 500);
        }
    ...
    return (
        <Modal
            ...
            backdrop="static"
        >
            {/* ✅ Only render GuideCue when portalContainer is available */}
            {portalContainer && currentConfig && (
                <GuideCueContainer
                    config={currentConfig}
                    feature={feature}
                    ready={openedInvoice !== null}
